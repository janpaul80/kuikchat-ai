...
export default function StatusPage() {
  ...
  const handlePostStatus = (e: React.FormEvent) => {
    ...
    const newStatus: Status = {
      ...
      viewers: ['Jane Doe', 'Alice Carter'], // Mock views
      isMine: true,
    }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background text-foreground md:flex-row">
              ...
              myStatuses.map((s) => (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className={`h-8 w-8 rounded-full bg-gradient-to-tr ${s.background} shrink-0`} />
            ...
            {statuses.length === 0 ? (
              ...
              statuses.map((s) => (
                <div
                  ...
                  onClick={() => setActiveStatusId(s.id)}
                >
                  <div className={`h-9 w-9 rounded-full bg-gradient-to-tr ${s.background} shrink-0 ring-2 ring-brand-blue-500 ring-offset-2 ring-offset-card`} />
                  <div className="min-w-0">
      ...
      <div className="flex-1 bg-black/95 flex flex-col items-center justify-center p-6 relative">
        {activeStatus ? (
          <div className="w-full max-w-sm aspect-[9/16] rounded-2xl bg-gradient-to-tr overflow-hidden flex flex-col justify-between p-6 shadow-2xl relative" style={{ backgroundImage: `linear-gradient(to top right, ${activeStatus.background.split(' ').map((c) => c.replace('from-', '').replace('to-', '')).join(', ')})` }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            <form onSubmit={handlePostStatus} className="space-y-4">
              <div className={`w-full aspect-[4/3] rounded-xl bg-gradient-to-tr ${activeGradient} p-4 flex flex-col justify-between`}>
                <textarea
                  value={statusText}
                ...
                <div className="flex gap-2">
                  {GRADIENTS.map((g) => (
                    <button
                      ...
                      className={`h-8 w-8 rounded-full bg-gradient-to-tr ${g} ring-offset-background transition-all ${
